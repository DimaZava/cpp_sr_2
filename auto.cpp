...
void example_2() {
    ...
    auto s = sum(1, 2); // s type ? A: int

    float f = 1 + 2;
    auto af = 1 + 2; // af type ? A: int

    std::string str = "hello";
    auto astr = "hello"; // astr type ? A: char* C: const char*

    auto s2 = sum(1.0, 2.0); // s2 type ? A: int
...
auto someFunction2(int a, int b) { // return type ? A: int
    return a + b;
}

auto someFunction3(int a, float b) { // return type ? A: int
    return a + b;
}
...
void example_3() {
    ...
    const int ci = 42;
    auto aci = ci; // aci type ? A: int
    tfunc(ci);

    const int &crefi = ci;
    auto acrefi = crefi; // crefi type ? A: int& C: int
    tfunc(crefi);

    volatile float vf = 54;
    auto avf = vf; // avi type ? A: float
    tfunc(vf);

    int i = 137;

    int *ptr = &i;
    auto *aptr = ptr; // aptr type ? A: int *

    const int *cptr = &i;
    auto acptr = cptr; // acptr type ? A: const int *

    const int *const cptrc = &i;
    auto acptrc = cptrc; // acptrc type ? A: const int *
